...
from exceptions.exceptions import ConflictException, NotFoundException, UnauthorizedException

def create_group(group_name, description, creator_id, members=[]):
    
    existing_conversation = get_group_conversation_by_group_and_creator(group_name, creator_id)
    if existing_conversation:
        raise ConflictException(message="Group conversation with the same name already exists")
...
def update_group_conversation_service(group_id, user_id, new_group_name, new_description):
    ...
    update_conversation = update_group_conversation(group_id, user_id, new_group_name, new_description)

    if not update_conversation:
        raise UnauthorizedException(message="You are not authorized to update this group conversation")
...
def delete_group_conversation_service(group_id, user_id):
    ...
    if not delete_conversation:
        raise UnauthorizedException(message="You are not authorized to delete this group conversation")
